...
## Overview
A tool designed to gather data from [Agmarknet](https://agmarknet.gov.in/) which is a government service that collates Agricultural Market information. The current implementation employs a direct URL substitution approach that aims to minimize interactions with the website for greater stability, and moves the relevant data collected to a Postgres database (as default behavior).
